...
```
When interpreting the output of the ANOVA table, it shows that the full model has the lowest RSS. This means that the full model fits the training data better than the other models since it has the lowest deviations from the data points. Accordingly, we should go with the full model.
...
data.frame(AIC_ALL = AIC_all,
           AIC_FORWARD = AIC_forward,
           AIC_LASSO = AIC_lasso)  
```

Surprisingly, when comparing the values of the AIC, the full model again has the lowest value. This is remarkable since the AIC punishes for adding variables to a model. However, according to the AIC, we should go with the full model.
```{r, comment=NA}
# Explained variance (r-squared)
r_all <- summary(model_all)$r.squared
r_forward <- summary(model_forward)$r.squared
r_lasso <- summary(model_lasso)$r.squared

data.frame(R2_ALL = r_all,
           R2_FORWARD = r_forward,
           R2_LASSO = r_lasso)
```
...
```
<br>
Even though the differences are hard to observe. When comparing the MSE's of the test data concerning model performance, the full model has the lowest value for MSE. This means that when given new data, the predictions for BMI of the full model come closest to the real observations of BMI. Thus, given the MSE value in the test data, we should again go for the full model.
...
**Yet**, since we're not interested in a model that contains all variables, we will further elaborate on our second best model: *forward stepwise selection*
